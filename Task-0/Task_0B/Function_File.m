...
function jacobian_matrices = find_jacobian_matrices(x_1, x_2, x1_dot, x2_dot)
  ...
  ################## ADD YOUR CODE HERE ######################
  jackkk=[];
  jack = jacobian([x1_dot;x2_dot],[x1;x2])
  for i=1:length(x_1)
      jaike =subs(jack,[x1 x2],[x_1(i) x_2(i)]);
      jaike=double(jaike);
      #jacobian_matrices{1,end+1}=(jaike);
      jackkk{1,end+1}=(jaike)
      
endfor
jacobian_matrices=([jackkk])
  ############################################################  
endfunction
...
#    eigen_values=double(eig(matrix))
#    m=real(q);
#    n=real(r);
#    if  real(eigen_values)< 0 
#       flag=1;
#    else 
#      flag =0;
#    endif
